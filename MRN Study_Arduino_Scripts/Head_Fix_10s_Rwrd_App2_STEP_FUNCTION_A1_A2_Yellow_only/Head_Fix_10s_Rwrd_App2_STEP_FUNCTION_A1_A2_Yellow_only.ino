...
if (Start == 1) {
  ...
  if (millis() - Delay >= 10000 && Z == 0){ 
    // 10000 for Laser at US onset; 8000 for Laser starts 2sec before US onset
   Z = 1; 
   LightDelay = millis();
   digitalWrite(10, HIGH);// 8 for blue
  }

if (Z == 1){
//  if (millis() - LightDelay <= 500){
//    digitalWrite(8, HIGH);
//  }
  if (millis() - LightDelay >= 500 ){
    digitalWrite(10, LOW);    // 8 for blue
  }
  ...
  if (millis() - Delay >= 10000){
    RWD=1;
//    digitalWrite(10, HIGH); // uncomment for 2s before US condition
    Serial.print("5000");
    Serial.print(",");
    Serial.print(millis()-StartStamp);
    Serial.print(",");
