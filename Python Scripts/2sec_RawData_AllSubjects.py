...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  3 07:46:20 2020
...
for FileE in glob.glob('*.csv'):
   ...
   ## f = open('200609-304-TrD4.csv', 'r')
    ...
    binSem = []
    binBIN = []
    for secBin in range(0,6):
        binSum.append(np.average(psth[:,secBin*2000:secBin*2000+999])*1000)
        binSem.append(stats.sem(np.sum(psth[:,secBin*2000:secBin*2000+999], axis=1)))
        binBIN.append(np.sum(psth[:,secBin*2000:secBin*2000+999]))
    Table.append(binSum)
    
        
        
    x = np.linspace(-6000, 4000, 6)
    ...
    XVal = numpy.append(XVal, timePSTH)            
    YVal= numpy.append(YVal, trialPSTH)

    
   
      
FileE_mod1 = FileE.strip('.csv') + '_event.csv'
with open(FileE_mod1, "a") as output:
    writer = csv.writer(output)
    writer.writerows(Table)
    
    
"""
FileE_mod2 = FileE.strip('.csv') + '_FULL.csv'
with open(FileE_mod2, "a") as output:
    writer = csv.writer(output)
    writer.writerow(XVal)
    writer.writerow(YVal)
"""
